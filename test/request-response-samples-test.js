    ...
    req.on("end", function () {
      ...
      if (requestContext.responseHttpHeaders) {
        for (const headerKey in requestContext.responseHttpHeaders) {
          res.setHeader(
            headerKey,
            requestContext.responseHttpHeaders[headerKey]
          );
        }
      }
...
tests.forEach(function (test) {
  ...
  if (fs.existsSync(responseHttpHeaders))
    responseHttpHeaders = require(responseHttpHeaders);
  else responseHttpHeaders = null;

  //attachmentParts
  if (fs.existsSync(attachmentParts))
    attachmentParts = require(attachmentParts);
...
function generateTest(
  ...
  suite[name] = function (done) {
    ...
    if (responseXML) {
      if (wsdlOptions.parseResponseAttachments) {
        //all LF to CRLF
      ...
      if (requestJSON) {
        if (err) {
          ...
          if (client.lastResponseAttachments) {
            assert.deepEqual(
              client.lastResponseAttachments.parts,
              attachmentParts
            );
          }
        }
