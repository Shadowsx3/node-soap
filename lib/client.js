...
class Client extends events_1.EventEmitter {
    ...
    _promisifyMethod(method) {
        return (args, options, extraHeaders) => {
            return new Promise((resolve, _reject) => {
                const callback = (err, result, soapHeader, httpHeader, soapResponse, rawResponse, rawRequest, mtomAttachments) => {
                    resolve({
                        err,
                        result,
                        rawResponse,
    ...
    _defineMethod(method, location) {
        ...
        return (args, callback, options, extraHeaders) => {
            ...
            this._invoke(method, args, location, (error, result, soapHeader, httpHeader, soapResponse, rawResponse, rawRequest, mtomAttachments) => {
                callback(error, result, soapHeader, httpHeader, soapResponse, rawResponse, rawRequest, mtomAttachments);
    ...
    _invoke(method, args, location, callback, options, extraHeaders) {
        ...
        const finish = (obj, body, response) => {
            ...
            if (!output) {
                ...
                return callback(null, null, obj.Header, response && response.headers, obj, response, xml, response.mtomResponseAttachments);
            }
            ...
            if (!obj.html && !obj.Body) {
                if (response.status >= 400) {
                    ...
                    return callback(error, obj, obj.Header, response && response.headers, obj, response, xml);
                }
                return callback(null, obj, obj.Header, response && response.headers, obj, response, xml);
            }
            ...
            callback(null, result, obj.Header, response && response.headers, obj, response, xml, response.mtomResponseAttachments);
        };
        const parseSync = (body, response) => {
            ...
            try {
                ...
                if (!output || !output.$lookupTypes) {
                    ...
                    if (json) {
                        return callback(null, response, obj.Header, response && response.headers, json, response, xml);
                    }
                }
                error.response = response;
                error.body = body;
                this.emit("soapError", error, eid);
                return callback(error, response, undefined, response && response.headers, response, response, xml, response.mtomResponseAttachments);
            }
        ...
        if (this.streamAllowed &&
            ...
            const onError = (err) => {
                ...
                if (this.returnSaxStream || !err.response || !err.response.data) {
                    callback(err, undefined, undefined, undefined, undefined, undefined, xml);
                }
                else {
            ...
            this.httpClient
                .requestStream(location, xml, headers, options, this)
                .then((res) => {
                ...
                if (res.status !== 200 || !output || !output.$lookupTypes) {
                    getStream(res.data).then((body) => {
                        ...
                        this.lastRequestHeaders =
                            (res.config && res.config.headers) || res.headers;
                ...
                if (this.returnSaxStream) {
                    ...
                    this.wsdl.xmlToObject(res.data, (error, obj) => {
                        ...
                        if (error) {
                            ...
                            error.body = "<stream>";
                            this.emit("soapError", error, eid);
                            return callback(error, res, obj.Header, res && res.headers, obj, res, xml);
                        }
        ...
        return this.httpClient.request(location, xml, (err, response, body) => {
            ...
            if (err) {
                ...
                callback(err, undefined, undefined, response && response.headers, undefined, response, xml);
            }
            else {
