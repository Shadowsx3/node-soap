...
class MessageElement extends Element {
    ...
    postProcess(definitions) {
        ...
        if (part.$element) {
            ...
            this.element.typeName = nsName.name;
            ...
            if (this.element.$type) {
                ...
                const typeNs = (schema.xmlns && schema.xmlns[type.prefix]) ||
                    definitions.xmlns[type.prefix];
                if (typeNs) {
                    if (type.name in Primitives) {
                        ...
                        const ctype = schema.complexTypes[type.name] ||
            ...
            if (tag === "binding") {
                if (child.name == "input" || child.name == "output") {
                    for (let i = 0, op; (op = child.children[i]); i++) {
                        if (op.name == "header") {
                            const messageName = (0, utils_1.splitQName)(op.$message).name;
